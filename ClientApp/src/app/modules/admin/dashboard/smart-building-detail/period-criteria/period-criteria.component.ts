...
export class PeriodCriteriaComponent implements OnInit {
  ...
  setPeriod() {
    if(this.selectedPeriod == 'Week') {
      ...
      this.endDate = moment().week(this.currentWeek).endOf('isoWeek').hour(24).minute(59).second(0).toDate();
    } else if(this.selectedPeriod == 'Day') {
      this.startDate = moment(this.currentDay).hour(1).minute(0).second(0).toDate();
      this.endDate = moment(this.currentDay).hour(24).minute(59).second(0).toDate();
    } else if(this.selectedPeriod == 'Month') {
      this.startDate = moment().year(this.currentYear).month(this.currentMonth).startOf('month').hour(1).minute(0).second(0).toDate();
      this.endDate = moment().year(this.currentYear).month(this.currentMonth).endOf('month').hour(24).minute(59).second(0).toDate();
    } else if(this.selectedPeriod == 'Year') {
      this.startDate = moment().year(this.currentYear).startOf('year').hour(1).minute(0).second(0).toDate();
      this.endDate = moment().year(this.currentYear).endOf('year').hour(24).minute(59).second(0).toDate();
    }
